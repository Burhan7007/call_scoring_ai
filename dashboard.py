...
def _safe_json_load(p: Path):
    try:
        ...
        logging.warning(f"⚠️ Could not load JSON {p.name}: {e}")
        return None
...
def _pretty_duration(dur):
    if isinstance(dur, dict):
        return _format_mmss(dur.get("talk_time", dur.get("total", 0)))
    try:
...
def dashboard():
    ...
    for r in all_calls:
        if q and q not in (r["translation"]["english"] or "").lower():
            continue
    ...
    agents = sorted({r.get("agent_name") for r in all_calls if r.get("agent_name")})
    langs = sorted({r.get("language_detected") for r in all_calls if r.get("language_detected")})
    statuses = sorted({r.get("call_status") for r in all_calls if r.get("call_status")})
