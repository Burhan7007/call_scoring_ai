...
@torch.inference_mode()
def _translate(text: str, name: str) -> str:
    tok, mdl = _load_translator(name)
    batch = tok([text], return_tensors="pt", padding=True, truncation=True)
    out = mdl.generate(**batch, max_new_tokens=512)
    return tok.decode(out[0], skip_special_tokens=True)
...
# ============================================================
# HELPERS – LANGUAGE / DURATION / ORDER STATUS
# ============================================================
def detect_language_from_country(phone: str) -> str:
    if not phone:
        return "en"
    s = str(phone).lstrip("+")
    mapping = {
...
def format_duration(dur):
    """
    Voiso duration formats:
      - plain seconds (int)
      - dict: {"total": 118, "talk_time": 108, ...}
    We show talk_time as mm:ss if possible.
    """
    if isinstance(dur, dict):
        talk = dur.get("talk_time") or dur.get("total") or 0
    elif isinstance(dur, (int, float, str)) and str(dur).isdigit():
        talk = int(dur)
    else:
        return str(dur)
...
def map_order_status(order_status: str) -> str:
    """
    Normalize internal order_status to a stable label.
    Kept simple on purpose so we don't override AI decisions.
    """
    status = (order_status or "").strip().lower()
    if status in ("accepted", "refused", "recall"):
        return status
    return "unknown"


# ============================================================
# BASIC KPI SCORING (USED AS BACKUP / SIMPLE SCORE)
# ============================================================
def score_text_basic(english_text: str):
    t = " " + (english_text or "").lower() + " "
    kpis = {
        "Greeting": [" hello ", " hi ", " good morning ", " good afternoon "],
        "Introduction": [" my name is ", " this is "],
...
def diarize(raw, pause=1.0, max_agent_run=4):
    ...
    - Splits on long pauses
    - Alternates Agent/Client in a stable way
    """
    dialogue, buf, cur, start, last, run_len = [], [], "Agent", 0.0, 0.0, 0

    def clean_repetition(t: str) -> str:
        words = t.split()
        cleaned = []
        last_word = ""
        for w in words:
...
def score_text(english_text: str):
    """
    AI-driven KPI scoring using semantic similarity (Milestone 1).
    """
    t = (english_text or "").lower()
    ...
    kpi_desc = {
        "Greeting": "agent greets politely with hello or good morning",
        "Introduction": "agent introduces themselves with their name",
        "Company Presentation": "agent presents the company",
        "Product Mention": "agent describes the product or order",
        "Upsell Product": "agent offers additional product or upgrade",
        "Insurance Upsell": "agent offers warranty or guarantee",
        "Address Confirmation": "agent confirms the delivery address",
        "Recap": "agent summarizes the order before ending the call",
        "Tone of Voice": "agent speaks politely and thanks the customer",
    }
    ...
    for kpi, desc in kpi_desc.items():
        try:
            e1 = embedder.encode([t], convert_to_tensor=True)
            e2 = embedder.encode([desc], convert_to_tensor=True)
    ...
    return ai_score, missing, comment
...
def change_password():
    ...
    if request.method == "POST__":
        # Note: typo safety, but keep as in spec: request.method == "POST"
        pass  # this won't run; fix below
...
def home():
    ...
    for f in RECORDINGS_DIR.glob("call_*.json"):
        try:
            ...
            d["agent_name"] = d.get("agent_name") or "Unknown"
            d["customer_phone"] = d.get("customer_phone") or "Unknown"
            d["language_detected"] = d.get("language_detected") or "Unknown"
            d["call_status"] = d.get("call_status") or "Unknown"

            # duration formatting for UI
            d["duration_display"] = format_duration(d.get("duration"))

            order_status_val = (d.get("order_status") or "").strip()
            rejection_val = (d.get("rejection_reason") or "").strip()

            d["order_status"] = order_status_val or "-"
            d["rejection_reason"] = rejection_val or "-"
            d["_order_status_norm"] = order_status_val.lower()
            d["_rejection_norm"] = rejection_val.lower()
...
def report_pdf(cid):
    ...
    c.setFont("Helvetica", 11)
    line(f"Agent: {d.get('agent_name')} | Customer: {d.get('customer_phone')}")
    line(f"Language: {d.get('language_detected')} | Duration: {format_duration(d.get('duration'))} | Status: {d.get('call_status')}")
    line(f"Order Status: {d.get('order_status', '-')}")
    line(f"Rejection: {d.get('rejection_reason', '-')}")
